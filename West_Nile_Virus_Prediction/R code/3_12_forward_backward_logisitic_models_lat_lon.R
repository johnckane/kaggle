...
backward_model <- glm(data=training_final,
                      wnvpresent ~ cut + species2 + longitude + avg_may_temp + num_may_rain +
...
#0.82888, just lessthan backward
...
write.csv(forward_submission,"forward_submission_lon.csv", row.names = FALSE)
write.csv(backward_submission,"backward_submission_lon.csv", row.names = FALSE)