...
test_predict <- predict(object = gam_fit,
                        newdata = test_final,
                        type = 'response')
gam_submission_fwd_vars <- data.frame(test_final$id, test_predict)
colnames(gam_submission_fwd_vars) <- c("Id","WnvPresent")
write.csv(gam_submission_fwd_vars, "gam_submission_forward_vars.csv", row.names = FALSE)
